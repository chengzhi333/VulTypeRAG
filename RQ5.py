...
def cot_predict_cwe(query_code, query_desc, retrieved_samples):
    ...
    #     "Your task is to infer the CWE type of the target vulnerability step by step based on similar samples.\n\n"
    # )
    prompt = (
        "You are an expert in software vulnerability type classification and CWE analysis. "
    ...
    # prompt += f"Step 3: Infer the CWE type of the target vulnerability.\n"
    ...
    # prompt += f"Analysis to determine the most likely CWE type of the target vulnerability.\n"
    # prompt += f"based on the reasoning in Step 2.\n\n"


    prompt += f"Output only a single specific CWE ID (e.g., CWE-xxx).\n"
    # prompt += f"Step 4: Output only a single specific CWE ID (e.g., CWE-xxx).\n"
