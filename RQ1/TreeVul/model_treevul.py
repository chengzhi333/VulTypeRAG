...
class ModelTreeVul(Model):
    def __init__(self,
                 ...
                 PTM: str = '/user/deeplearn/codebert-base',
                 dropout: float = 0.1,
                 label_file: str = 'data1valid_cwes.json',
                 cwe_path_file: str = 'data1/cwe_path.json',
        ...
        for depth in range(self._max_depth):
            self._metrics[f"depth-{depth}_accuracy"] = CategoricalAccuracy()
            self._metrics[f"depth-{depth}_fscore_weighted"] = FBetaMeasure(
                beta=1.0,
                average="weighted",
                labels=range(len(self._labels[depth]))
            )
    ...
    def forward(self,
        ...
        # 🟢🟢🟢 新增：更新所有 depth 的 metrics（训练和验证阶段都更新）🟢🟢🟢
        for depth in range(self._max_depth):
            if f"probs_{depth}" in output_dict and f"label_{depth}" in labels:
                self._metrics[f"depth-{depth}_accuracy"](predictions=output_dict[f"probs_{depth}"], gold_labels=labels[f"label_{depth}"])
                self._metrics[f"depth-{depth}_fscore_weighted"](predictions=output_dict[f"probs_{depth}"], gold_labels=labels[f"label_{depth}"])
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        for d in range(self._max_depth):
            try:
                # Accuracy (保留原格式)
                acc_key = f"depth-{d}_accuracy"
                if acc_key in self._metrics:
                    metrics[f"depth-{d}_accuracy"] = self._metrics[acc_key].get_metric(reset)

                # Weighted F-score (输出简洁格式)
                fscore_key = f"depth-{d}_fscore_weighted"
                if fscore_key in self._metrics:
                    scores = self._metrics[fscore_key].get_metric(reset)
                    
                    # 兼容 dict 格式：{'precision': p, 'recall': r, 'fscore': f}
                    if isinstance(scores, dict):
                        precision = scores.get('precision', 0.0)
                        recall = scores.get('recall', 0.0)
                        fscore = scores.get('fscore', 0.0)
                    # 兼容 tuple/list 格式
                    else:
                        values = list(scores.values()) if hasattr(scores, 'values') else list(scores)
                        precision = values[0] if len(values) > 0 else 0.0
                        recall = values[1] if len(values) > 1 else 0.0
                        fscore = values[2] if len(values) > 2 else 0.0

                    metrics[f"{d}_weighted_precision"] = precision
                    metrics[f"{d}_weighted_recall"] = recall
                    metrics[f"{d}_weighted_fscore"] = fscore

            except Exception as e:
                print(f"[WARNING] Failed to get metrics for depth-{d}: {e}")
                continue

        # 处理自定义指标（如 overall_hierarchy）
        try:
            custom_result = self._custom_metrics.get_metric(reset)
            if isinstance(custom_result, dict):
                for key, value in custom_result.items():
                    metrics[key] = value
            else:
                print(f"[WARNING] custom_metrics returned non-dict: {type(custom_result)}")
        except Exception as e:
            print(f"[WARNING] Failed to get custom metrics: {e}")
