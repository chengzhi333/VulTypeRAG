...
def _test(archive_file,
         ...
         package="TreeVul",
         prepare_first=True,
         cwe_tree_path="data1/valid_cwe_tree.json",
         batch_weight_key="",
...
def TreeVul_test(test_set, package, model, weights=None, cuda=0, seed=2024):
    # wrapping api for test

    if "bilstm" in model or "codebert" in model:
    ...
    cwe_tree_path = f"{DATA_PATH}/data1/valid_cwe_tree.json"
    if not os.path.exists(f"{MODEL_PATH}/code_message_lzq1/"):
        os.mkdir(f"{MODEL_PATH}/code_message_lzq1/")
    output_file = f"{MODEL_PATH}/code_message_lzq1/{model}_metric.json"
    predictions_output_file = f"{MODEL_PATH}/code_message_lzq1/{model}_result.json"
    
    _test(archive_file=f"{MODEL_PATH}/{model}/model.tar.gz", input_file=f"{DATA_PATH}/data1/{test_set}.json", 
...
DATA_PATH = "/user/deeplearn"
MODEL_PATH = "/user/deeplearn/TreeVul"

if __name__ == "__main__":
    os.chdir("/user/deeplearn")
    test_set = "test_treevul(2)"
    package = "TreeVul"
    # package = "Baseline"
    model = "out_treevul6"
    ...
    TreeVul_test(test_set=test_set, package=package, model=model, weights=weights, cuda=cuda, seed=seed)