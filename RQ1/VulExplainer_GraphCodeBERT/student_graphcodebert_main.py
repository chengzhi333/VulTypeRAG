...
# Ensure the local parser package is used instead of the stdlib module.
_current_dir = Path(__file__).resolve().parent
_parser_init = _current_dir / "parser" / "__init__.py"
if "parser" not in sys.modules:
    if not _parser_init.exists():
        raise ImportError(f"Cannot find local parser package at {_parser_init}")
    _spec = importlib.util.spec_from_file_location("parser", _parser_init)
    _parser_module = importlib.util.module_from_spec(_spec)
    sys.modules["parser"] = _parser_module
    _spec.loader.exec_module(_parser_module)  # type: ignore

from parser import DFG_csharp, DFG_cpp
...
CODE_LANGUAGE = os.environ.get("CODE_LANGUAGE", "c_sharp")
if CODE_LANGUAGE not in SUPPORTED_LANGUAGES:
    raise ValueError(f"Unsupported CODE_LANGUAGE '{CODE_LANGUAGE}'. Supported: {list(SUPPORTED_LANGUAGES.keys())}")
dfg_function={
    CODE_LANGUAGE:SUPPORTED_LANGUAGES[CODE_LANGUAGE],
}
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for i in range(len(y_preds)):
        ...
        precision_macros.append(precision_score(y_trues[i], y_preds[i], average='macro', zero_division=0))
        recall_macros.append(recall_score(y_trues[i], y_preds[i], average='macro', zero_division=0))
        f1_macros.append(f1_score(y_trues[i], y_preds[i], average='macro', zero_division=0))
        precision_weighteds.append(precision_score(y_trues[i], y_preds[i], average='weighted', zero_division=0))
        recall_weighteds.append(recall_score(y_trues[i], y_preds[i], average='weighted', zero_division=0))
        f1_weighteds.append(f1_score(y_trues[i], y_preds[i], average='weighted', zero_division=0))
        mccs.append(matthews_corrcoef(y_trues[i], y_preds[i]))
    ...
    result = {
        "eval_acc": float(acc),
        "eval_precision_macro": float(precision_macros[max_idx]),
        "eval_recall_macro": float(recall_macros[max_idx]),
        "eval_f1_macro": float(f1_macros[max_idx]),
        "eval_precision_weighted": float(precision_weighteds[max_idx]),
        "eval_recall_weighted": float(recall_weighteds[max_idx]),
        "eval_f1_weighted": float(f1_weighteds[max_idx]),
        "eval_mcc": float(mccs[max_idx]),
        "best_beta": best_beta,
    }
...
def test(args, model, tokenizer, test_dataset, beta=None, cwe_label_map=None):
    # build dataloader
    ...
    precision_weighted = precision_score(y_trues, y_preds, average='weighted', zero_division=0)
    recall_weighted = recall_score(y_trues, y_preds, average='weighted', zero_division=0)
    ...
    result = {
        ...
        "eval_precision_weighted": float(precision_weighted),
        "eval_recall_weighted": float(recall_weighted),
        "eval_f1_weighted": float(f1_weighted),
        "eval_mcc": float(mcc),
        "best_beta": BEST_BETA,
    }
    ...
    if cwe_label_map is not None:
        try:
            # reverse map class index -> cwe id
            idx_to_cwe = {}
            for cwe, v in cwe_label_map.items():
                idx_to_cwe[v[0]] = cwe
            true_cwe = [idx_to_cwe.get(i, "") for i in y_trues]
            pred_cwe = [idx_to_cwe.get(i, "") for i in y_preds]
            df_out = pd.DataFrame({
                "true_index": y_trues,
                "pred_index": y_preds,
                "true_cwe": true_cwe,
            ...
            out_path = os.path.join(args.output_dir, "test.csv")
            df_out.to_csv(out_path, index=False)
            logger.info("Saved test predictions to %s", out_path)
        except Exception as e:
            logger.warning("Failed to save test predictions: %s", str(e))
...
def main():
    ...
    if args.do_test:
        ...
        y_trues, y_preds = test(args, model, tokenizer, test_dataset, beta=args.beta, cwe_label_map=cwe_label_map) 
