...
def _get_child_by_any_field(node, field_names):
    """Return the first child that matches one of the given field names."""
    if node is None:
        return None
...
def DFG_cpp(root_node,index_to_code,states):
    ...
    def_statement=['init_declarator','parameter_declaration','structured_binding_declarator']
    increment_statement=['update_expression']
    if_statement=['if_statement','else_clause']
    for_statement=['for_statement','range_based_for_loop','range_for_statement']
    ...
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment':
        ...
        name=_get_child_by_any_field(root_node,['declarator','name','left'])
        value=_get_child_by_any_field(root_node,['value','initializer','right','default_value'])
        DFG=[]
        if value is not None:
            temp,states=DFG_cpp(value,index_to_code,states)
            DFG+=temp
        target_node=name if name is not None else root_node
        name_indexs=tree_to_variable_index(target_node,index_to_code)
        if value is None:
        ...
        left=_get_child_by_any_field(root_node,['left','declarator','name'])
        right=_get_child_by_any_field(root_node,['right','value','initializer'])
        if right is None and len(root_node.children)>=2:
            right=root_node.children[-1]
        if left is None and len(root_node.children)>=1:
            left=root_node.children[0]
        DFG=[]
        if right is not None:
            temp,states=DFG_cpp(right,index_to_code,states)
            DFG+=temp
        left_tokens_index=tree_to_variable_index(left,index_to_code) if left is not None else tree_to_variable_index(root_node,index_to_code)
        right_tokens_index=tree_to_variable_index(right,index_to_code) if right is not None else []
        for token1_index in left_tokens_index:
            idx1,code1=index_to_code[token1_index]
