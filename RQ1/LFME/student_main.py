...
def analyze_predictions(y_trues, y_preds, cwe_label_map, args):
    ...
    groups = ["1", "2", "3"]
    for g in groups:
        df = pd.read_csv(f"g{g}/g{g}_test.csv")
        classes = df["type"].tolist()
        ...
        print(f"(G{g}) Accuracy: ", group_accuracy)
...
def main():
    ...
    if args.do_test:
        ...
        y_trues, y_preds = test(args, student_model, tokenizer, test_dataset, cwe_label_map)
    return results
