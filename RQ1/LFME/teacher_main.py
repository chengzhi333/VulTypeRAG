...
def resolve_label_key(raw_label, cwe_label_map):
    """
    Normalize labels so both numeric IDs (e.g. 787) and strings such as
    'CWE-787' match the keys stored in cwe_label_map.
    """
    candidates = []
    if isinstance(raw_label, str):
        candidates.append(raw_label.strip())
    elif isinstance(raw_label, (int, float)) and not np.isnan(raw_label):
        candidates.append(str(int(raw_label)))
    else:
        candidates.append(str(raw_label))
    ...
    for cand in normalized:
        if cand not in seen:
            seen.add(cand)
            search_list.append(cand)

    for cand in search_list:
        if cand in cwe_label_map:
            return cand

    sample_keys = list(cwe_label_map.keys())[:5]
    raise KeyError(f"Label '{raw_label}' not found in cwe_label_map keys (sample: {sample_keys})")
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, cwe_label_map, file_type="train"):
        ...
        for i in tqdm(range(len(funcs))):
            label_key = resolve_label_key(labels[i], cwe_label_map)
            label = cwe_label_map[label_key][1]
            # count label freq if it's training data
            if file_type == "train":
                cwe_label_map[label_key][2] += 1
