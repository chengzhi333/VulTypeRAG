...
def test(args, model, tokenizer, test_dataset, cwe_label_map):
    # build dataloader
...
def main():
    ...
    # Try to load tokenizer with multiple fallback options using AutoTokenizer
    try:
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name)
        logger.info("Successfully loaded tokenizer from %s", args.tokenizer_name)
    except Exception as e:
        logger.warning("Failed to load tokenizer from %s: %s", args.tokenizer_name, str(e))
        logger.info("Trying to load tokenizer from model path instead...")
        try:
            # Try loading from model path
            tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
            logger.info("Successfully loaded tokenizer from model path %s", args.model_name_or_path)
        except Exception as e2:
            logger.warning("Failed to load tokenizer from model path %s: %s", args.model_name_or_path, str(e2))
            logger.info("Trying alternative model names...")
            # Try alternative model names
            alternative_names = [
                "claudios/CodeGPT-small-java-adaptedGPT2",
                "microsoft/CodeGPT-small-java",
                "gpt2"  # Fallback to standard GPT2
            ]
            tokenizer = None
            for alt_name in alternative_names:
                try:
                    tokenizer = AutoTokenizer.from_pretrained(alt_name)
                    logger.info("Successfully loaded tokenizer from %s", alt_name)
                    break
                except Exception as ex:
                    logger.warning("Failed to load tokenizer from %s: %s", alt_name, str(ex))
                    continue
            if tokenizer is None:
                raise RuntimeError(f"Failed to load tokenizer from any source. Please check your model path or internet connection.")
    
    # GPT-2 doesn't have a pad_token by default, so we need to set it
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id
        logger.info("Set pad_token to eos_token (GPT-2 requirement)")
    ...
    # Load model with error handling using AutoModel
    try:
        gpt = AutoModel.from_pretrained(args.model_name_or_path)
        logger.info("Successfully loaded model from %s", args.model_name_or_path)
    except Exception as e:
        logger.warning("Failed to load model from %s: %s", args.model_name_or_path, str(e))
        logger.info("Trying alternative model names...")
        # Try alternative model names
        alternative_names = [
            "claudios/CodeGPT-small-java-adaptedGPT2",
        ...
        for alt_name in alternative_names:
            try:
                gpt = AutoModel.from_pretrained(alt_name)
                logger.info("Successfully loaded model from %s", alt_name)
                break
            except Exception as ex:
                logger.warning("Failed to load model from %s: %s", alt_name, str(ex))
                continue
        if gpt is None:
            raise RuntimeError(f"Failed to load model from any source. Please check your model path or internet connection.")
    ...
    if args.do_test:
        ...
        y_trues, y_preds = test(args, model, tokenizer, test_dataset, cwe_label_map)
    return results
