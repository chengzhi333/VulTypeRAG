...
import regvd_model as regvd_module
...
def test(args, model, tokenizer, test_dataset, cwe_label_map, best_threshold=0.5, out_suffix=None):
    # build dataloader
    ...
    try:
        ...
        name = "test.csv" if not out_suffix else f"test_task_{out_suffix}.csv"
...
def main():
    ...
    # Setup CUDA, GPU (safe selection of device 0 when CUDA is available)
    if torch.cuda.is_available():
        safe_device_index = 0
        try:
            torch.cuda.set_device(safe_device_index)
        except Exception:
            safe_device_index = 0
        device = torch.device(f"cuda:{safe_device_index}")
    ...
    # ensure submodules that reference a module-level `device` see the correct device
    try:
        regvd_module.device = args.device
    except Exception:
        pass
    ...
    if args.do_test:
        ...
        y_trues, y_preds = test(args, model, tokenizer, test_dataset, cwe_label_map, best_threshold=0.5, out_suffix="single")
    return results
