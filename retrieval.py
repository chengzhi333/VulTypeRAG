...
def cot_predict_cwe(query_code, query_desc, retrieved_samples):
    ...
    # prompt += f"Below are the retrieved vulnerability samples.\n"
    for i, s in enumerate(retrieved_samples):
        # === 跳过 Remain Class 层级关系 ===
        # if s["cwe_id"].upper().strip() == "REMAIN CLASS":
        #     s["hierarchy_text"] = "  (Remain Class has no hierarchy)"
        #     s["patch"] = "Remain Class"
        # else:
    ...
    # # 提取唯一CWE
    # unique_cwe_ids = list({s["cwe_id"] for s in retrieved_samples})
    # prompt += f"Below are the CWE hierarchical relationships corresponding to the CWE types of these samples.\n"
    # for cwe in unique_cwe_ids:
    #     hierarchy = query_cwe_hierarchy(cwe)
    #     hierarchy_text = build_hierarchy_text(hierarchy)
    #     if len(hierarchy) >= 2:
    #         patch = f"{hierarchy[-2]}->{hierarchy[-1]}"
    #     elif len(hierarchy) == 1:
    #         patch = hierarchy[0]
    #
    #     prompt += f"CWE-ID: {cwe}\n"
    #     prompt += f"- Patch: {patch}\n"
    #     prompt += f"- CWE Hierarchy:\n{hierarchy_text}\n\n"
    ...
    # prompt += f"Step 4: Special rule for 'Remain Class':\n"
    # prompt += f"If among the retrieved similar samples there exists one or more samples labeled as 'Remain Class',\n"
    # prompt += f"and the target vulnerability shows higher similarity with these 'Remain Class' samples\n"
    # prompt += f"than with samples having specific CWE types, then prioritize outputting 'Remain Class'.\n"
    # prompt += f"Otherwise, output the most appropriate specific CWE-ID.\n\n"
    ...
    #     model="deepseek-chat",
    #     messages=[
    #         {"role": "system", "content": "YYou are an expert in software vulnerability type identification."},
    #         {"role": "user", "content": prompt}
    #     ],
    #     temperature = 0,
    #     stream=False
...
def predict_cwe_rag_kg_cot(code, desc, ast):
    ...
    # 获取检索到的 CWE-ID 集合
    # retrieved_cwe_list = [s["cwe_id"] for s in samples]
    # return retrieved_cwe_list
...
if __name__ == "__main__":
    ...
    # if "Predicted_CWE" not in df.columns:
    #     df["Predicted_CWE"] = ""
    # if "Retrieved_CWE_Set" not in df.columns:
    #     df["Retrieved_CWE_Set"] = ""
    #
    # for idx, row in df.iterrows():
    #     if str(row["Predicted_CWE"]).strip() and str(row["Retrieved_CWE_Set"]).strip():
    #         continue
    #     try:
    #         code, desc, ast = row["func_before"], row["description"], row["ast"]
    #         # 仅获取检索到的相似样本 CWE 集
    #         retrieved_set = predict_cwe_rag_kg_cot(code, desc, ast)
    #         df.at[idx, "Retrieved_CWE_Set"] = ",".join(retrieved_set)
    #
    #         # 可选：调用原 cot 推理
    #         # pred = predict_cwe_rag_kg_cot(code, desc)
    #         # df.at[idx, "Predicted_CWE"] = pred
    #
    #         df.to_csv(output_file_with_set, index=False)
    #         print(f"[{idx}] Retrieved CWE Set: {retrieved_set}")
    #     except Exception as e:
    #         print(f"Error at row {idx}: {e}")
    #         continue
    #
    # print(f"✅ CWE set extraction completed. Saved to {output_file_with_set}")
