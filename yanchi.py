...
def rerank_by_code_similarity(test_code, test_ast_vec, candidate_data, topk):
    ...
    for item in candidate_data:
        ...
        # ast_sim = ast_similarity(test_ast_vec_as_string, ast_from_candidate)
...
def predict_cwe_rag_kg_cot(code, desc, ast):
    ...
    print(f"⏱️ RAG Retrieval Time: {retrieval_time:.4f} seconds")
    ...
    print(f"⏱️ LLM Inference Time: {llm_time:.4f} seconds")

    return predicted_cwe, retrieval_time, llm_time

# ================== 运行入口 ==================
if __name__ == "__main__":
    input_file = "output_dataset/Test_with_ast.csv"
    output_file = "prediction_data/test_predicted_cwe_yanchi.csv"
    ...
    if len(rows_to_predict) == 0:
        ...
        rows_to_predict = rows_to_predict[:10]  # ⭐ 仅测试前十个样本
        print(f"🎯 仅测试前十个样本: indices = {list(rows_to_predict)}")
        ...
        for idx in rows_to_predict:
            ...
            try:
                pred, retrieval_time, llm_time = predict_cwe_rag_kg_cot(code, desc, ast)
            ...
            df.at[idx, "LLM_Time"] = llm_time

            # 临时保存，防止写入中断损坏
            df.to_csv(temp_file, index=False)
            os.replace(temp_file, output_file)

        print(f"✅ 前十个样本预测完成，结果已保存到 {output_file}")